...
if(isset($_SESSION['category_id']) && $_SESSION['category_id']=="MASTER"){
    $stmt=$pdo->query("select * from movie where movie_name like '%".$_POST['key']."%'");
    ...
    $index=0;
    while($row=$stmt->fetch(PDO::FETCH_ASSOC)){
        $stmt2=$pdo->query("select movie_name from movie where movie_id='".$row['movie']."' and movie_name like  movie_name like '%".$_POST['key']."%'");
